...
//!
//! When using acute ECS, systems are usually not run directly, but are inserted into a
...
impl Schedule {
    ...
    pub fn add_system_to_stage<Params>(
        ...
        // Use a function instead of a closure to ensure that it is codegend inside acute_ecs instead
    ...
    pub fn run_once(&mut self, world: &mut World) {
        for label in &self.stage_order {
            #[cfg(feature = "trace")]
            let _stage_span = acute_utils::tracing::info_span!("stage", name = ?label).entered();
