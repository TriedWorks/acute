...
use acute_utils::{tracing::info, HashMap, HashSet};
...
impl SystemStage {
    ...
    pub fn apply_buffers(&mut self, world: &mut World) {
        for container in &mut self.parallel {
            ...
            #[cfg(feature = "trace")]
            let _span = acute_utils::tracing::info_span!("system_commands", name = &*system.name())
                .entered();
    ...
    fn run(&mut self, world: &mut World) {
        ...
        while run_stage_loop {
            ...
            for index in 0..self.run_criteria.len() {
                ...
                #[cfg(feature = "trace")]
                let _span =
                    acute_utils::tracing::info_span!("run criteria", name = &*criteria.name())
                        .entered();
            ...
            while run_system_loop {
                ...
                for container in &mut self.exclusive_at_start {
                    if should_run(container, &self.run_criteria, default_should_run) {
                        #[cfg(feature = "trace")]
                        let _system_span = acute_utils::tracing::info_span!(
                            "exclusive_system",
                            name = &*container.name()
                        )
                        .entered();
                ...
                if self.apply_buffers {
                    for container in &mut self.parallel {
                        if container.should_run {
                            #[cfg(feature = "trace")]
                            let _span = acute_utils::tracing::info_span!(
                                "system_commands",
                                name = &*container.name()
                            )
                            .entered();
...
mod tests {
    ...
    fn run_criteria_with_query() {
        use crate::{self as acute_ecs, component::Component};
