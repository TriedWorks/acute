...
use acute_tasks::{ComputeTaskPool, Scope, TaskPool};
#[cfg(feature = "trace")]
use acute_utils::tracing::Instrument;
...
impl ParallelSystemExecutor for ParallelExecutor {
    ...
    fn run_systems(&mut self, systems: &mut [ParallelSystemContainer], world: &mut World) {
        ...
        {
            #[cfg(feature = "trace")]
            let _span = acute_utils::tracing::info_span!("update_archetypes").entered();
        ...
        compute_pool.scope(|scope| {
            ...
            #[cfg(feature = "trace")]
            let span = acute_utils::tracing::info_span!("parallel executor");
            #[cfg(feature = "trace")]
...
impl ParallelExecutor {
    ...
    fn prepare_systems<'scope>(
        ...
        #[cfg(feature = "trace")]
        let _span = acute_utils::tracing::info_span!("prepare_systems").entered();
            ...
            self.system_metadata.iter_mut().zip(systems).enumerate()
        {
            ...
            if system.should_run() {
                ...
                let system_span =
                    acute_utils::tracing::info_span!("system", name = &*system.name());
                #[cfg(feature = "trace")]
                let overhead_span =
                    acute_utils::tracing::info_span!("system overhead", name = &*system.name());
                let task = async move {
                    start_receiver
                        .recv()
                        .await
