...
use acute_ptr::{OwningPtr, UnsafeCellDeref};
use acute_utils::tracing::debug;
use std::{
    any::TypeId,
    fmt,
...
impl Default for World {
    fn default() -> Self {
        Self {
            id: WorldId::new().expect("More `acute` `World`s have been created than is supported"),
...
impl World {
    ...
    ///
    /// ```
    /// use acute_ecs::{component::Component, world::World};
    ///
    /// #[derive(Component)]
    /// struct Position {
    ///   x: f32,
    ...
    ///
    /// ```
    /// use acute_ecs::{component::Component, entity::Entity, world::World};
    ///
    /// #[derive(Component)]
    ...
    /// run queries on the [`World`] by storing and reusing the [`QueryState`].
    /// ```
    /// use acute_ecs::{component::Component, entity::Entity, world::World, query::With};
    ///
    /// #[derive(Component)]
    /// struct A;
    /// #[derive(Component)]
    ...
    ///
    /// ```
    /// use acute_ecs::{entity::Entity, world::World, component::Component};
    /// #[derive(Component)]
    /// struct A(&'static str);
    /// #[derive(Component, PartialEq, Debug)]
    /// struct B(f32);
    ...
    /// # Example
    /// ```
    /// use acute_ecs::{component::Component, world::{World, Mut}};
    /// #[derive(Component)]
    /// struct A(u32);
    /// #[derive(Component)]
    /// struct B(u32);
...
// TODO: remove allow on lint - https://github.com/acuteengine/acute/issues/3666
...
mod tests {
    use super::World;
    use acute_ecs_macros::Component;
    use std::{
        panic,
        sync::{
    ...
    // For acute_ecs_macros
    use crate as acute_ecs;
