use acute_macro_utils::{get_lit_str, Symbol};
...
pub fn derive_component(input: TokenStream) -> TokenStream {
    ...
    let acute_ecs_path: Path = crate::acute_ecs_path();
    ...
    let storage = storage_path(&acute_ecs_path, attrs.storage);
    ...
    TokenStream::from(quote! {
        impl #impl_generics #acute_ecs_path::component::Component for #struct_name #type_generics #where_clause {
...
fn parse_component_attr(ast: &DeriveInput) -> Result<Attrs> {
    let meta_items = acute_macro_utils::parse_attrs(ast, COMPONENT)?;
...
fn storage_path(acute_ecs_path: &Path, ty: StorageTy) -> TokenStream2 {
    ...
    quote! { #acute_ecs_path::component::#typename }
}
