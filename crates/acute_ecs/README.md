...
[![Crates.io](https://img.shields.io/crates/v/acute_ecs.svg)](https://crates.io/crates/acute_ecs)
[![license](https://img.shields.io/badge/license-MIT-blue.svg)](https://github.com/acuteengine/acute/blob/HEAD/LICENSE)
[![Discord](https://img.shields.io/discord/691052431525675048.svg?label=&logo=discord&logoColor=ffffff&color=7389D8&labelColor=6A7EC2)](https://discord.gg/acute)

## What is acute ECS?

acute ECS is an Entity Component System custom-built for the [acute][acute] game engine. It aims to be simple to use, ergonomic, fast, massively parallel, opinionated, and featureful. It was created specifically for acute's needs, but it can easily be used as a standalone crate in other projects.

## ECS

All app logic in acute uses the Entity Component System paradigm, which is often shortened to ECS. ECS is a software pattern that involves breaking your program up into Entities, Components, and Systems. Entities are unique "things" that are assigned groups of Components, which are then processed using Systems.
...
acute ECS is acute's implementation of the ECS pattern. Unlike other Rust ECS implementations, which often require complex lifetimes, traits, builder patterns, or macros, acute ECS uses normal Rust data types for all of these concepts:
...
Systems are normal Rust functions. Thanks to the Rust type system, acute ECS can use function parameter types to determine what data needs to be sent to the system. It also uses this "data access" information to determine what Systems can run in parallel with each other.
...
Apps often require unique resources, such as asset collections, renderers, audio servers, time, etc. acute ECS makes this pattern a first class citizen. `Resource` is a special kind of component that does not belong to any entity. Instead, it is identified uniquely by its type:
...
Schedules consist of zero or more Stages, which run a set of Systems according to some execution strategy. acute ECS provides a few built in Stage implementations (ex: parallel, serial), but you can also implement your own! Schedules run Stages one-by-one in an order defined by the user.
...
acute ECS should feel very natural for those familiar with Rust syntax:
...
acute ECS tracks _all_ changes to Components and Resources.
...
acute ECS supports multiple component storage types.
