...
pub struct AcuteManifest {
    manifest: Map<String, Value>,
}

impl Default for AcuteManifest {
    fn default() -> Self {
        Self {
...
impl AcuteManifest {
    pub fn maybe_get_path(&self, name: &str) -> Option<syn::Path> {
        const ACUTE: &str = "acute";
        const ACUTE_INTERNAL: &str = "acute_internal";
        ...
        let find_in_deps = |deps: &Map<String, Value>| -> Option<syn::Path> {
            ...
            } else if let Some(dep) = deps.get(ACUTE) {
                dep_package(dep).unwrap_or(ACUTE)
            } else if let Some(dep) = deps.get(ACUTE_INTERNAL) {
                dep_package(dep).unwrap_or(ACUTE_INTERNAL)
            } else {
                return None;
            };

            let mut path = Self::parse_str::<syn::Path>(package);
            if let Some(module) = name.strip_prefix("acute_") {
