...
use crate::{
    ...
    scenes::{Scene, SceneHandler},
    tools::camera::{Camera, CameraController},
};
...
impl Acute {
    pub fn new(
        ...
        let renderer = futures::executor::block_on(Renderer::new(window, size));
    ...
    pub fn run(&mut self, event: &Event<()>, control_flow: &mut ControlFlow) {
        ...
        if self.timer.should_fixed_update() {
            ...
            self.renderer
                .update_render_data(&self.worlds[0], &self.camera);
        }
        match event {
